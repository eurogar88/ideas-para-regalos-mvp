...
for (const file of htmlFiles) {
  ...
  assert.match(html, /\/analytics\.js\?v=analytics-2/, `${relative} is missing shared analytics`);
  assert.match(html, /styles\.css\?v=growth-ui-13/, `${relative} points to an old stylesheet`);
...
assert.match(serviceWorker, /analytics\.js\?v=analytics-2/);
