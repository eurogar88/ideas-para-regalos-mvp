...
- La selección también puede convertirse en una tarjeta PNG lista para compartir por el móvil o guardar en el dispositivo; solo incluye ideas editoriales, etiquetas no identificativas y la URL pública.
...
- Bloque de descubrimiento semanal que rota automáticamente una idea distinta cada siete días, sin añadir un feed, una base de datos ni un deploy manual. Si está disponible la API autorizada, consulta un producto real para mostrar título, imagen, precio y ficha; conserva “Ver similares” como búsqueda alternativa y degrada a la búsqueda editorial si no hay ficha válida.
...
- catalog.js: catálogo editorial cargado de forma diferida para no bloquear el primer render de la portada.
...
La release incorpora clusters editoriales adicionales para hombre, mujer, adolescente, compañero de trabajo y alguien a quien se conoce poco. El hub y `llms.txt` los enlazan para facilitar descubrimiento orgánico y respuestas de asistentes sin crear páginas clonadas.
...
Eventos growth-v5: page_viewed (path), quiz_started, quiz_step_viewed (questionId, step), quiz_answered (questionId, step), quiz_abandoned (questionId, step), quiz_completed (genderProvided, interestCount), recommendations_viewed (resultCount, variant, mode), recommendations_refreshed (variant, mode), gift_outbound_clicked (giftId, position, store, mode, linkType), language_changed (from, to), share_clicked (mode, ideaCount), share_completed (method, mode), shared_result_opened (mode), weekly_discovery_viewed (giftId), weekly_discovery_clicked (giftId, store, linkType), weekly_discovery_product_ready (giftId, linkType), gift_impression (giftId, position), share_card_created (ideaCount), share_card_downloaded (ideaCount), pwa_ready, pwa_install_prompt_viewed, pwa_install_prompted, pwa_install_choice (outcome), pwa_installed, pwa_install_dismissed, quiz_reset, analytics_loaded y gift_feedback (giftId, feedback). Todos incluyen app, language, versión, ruta de entrada, campaña permitida y marca temporal. No se envían respuestas concretas, nombres, emails ni URLs compartidas; además, el SDK no conserva la URL completa en sus propiedades automáticas. `quiz_step_viewed` permite ver en qué paso se pierde la gente, `linkType` separa clics a producto de clics en “Ver similares” y las impresiones se limitan a un identificador editorial y una posición. El historial local de clics solo se guarda después de aceptar la analítica, y volver a aceptar después de retirarla reactiva correctamente el SDK.
...
Para campañas de La Mami Hacker se puede usar una URL con atribución, sin cambiar la página indexable: `https://regalazo.xyz/?utm_source=lamamihacker&utm_medium=instagram&utm_campaign=regalazo-launch`. La portada muestra un contexto breve para ese tráfico y Mixpanel conserva únicamente esos valores de campaña permitidos.
